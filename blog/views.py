...
def get_index(request):
    posts = Post.objects.filter(published_date__lte = timezone.now()) # Check whether or not the post is published (double underscore required).
...
def read_post(request, id):
    ...
    can_edit = user_can_edit_post(request, post) # Is the post author a user or a superuser or in the group "editors"
